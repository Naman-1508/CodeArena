                ...
                {(output.status === 'success' || (output.status === 'error' && output.details)) && (
                    <div className="space-y-6 max-w-4xl">
                      <div className="flex items-center justify-between">
                        <h2 className={`text-3xl font-black tracking-tight flex items-center gap-3 ${output.status === 'success' ? 'text-green-500' : 'text-red-500'}`}>
                          {output.status === 'success' ? (
                            <>
                        ...
                        {output.status === 'success' && output.earnedXp > 0 && (
                           <div className="flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/20 px-3 py-1.5 rounded-xl">
                              <Target className="w-4 h-4 text-yellow-500" />
                              <span className="text-yellow-500 font-bold text-sm">+{output.earnedXp} XP Earned</span>
                           </div>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         {/* Runtime Analytics */}
                         <div className="glass-panel border border-white/5 p-5 rounded-2xl relative overflow-hidden group">
                            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                               <Clock className="w-12 h-12 text-blue-400" />
                            </div>
                            <div className="text-[10px] uppercase font-black text-slate-500 tracking-widest mb-1">Runtime</div>
                            <div className="flex items-baseline gap-2 mb-3">
                               <span className="text-2xl font-black text-white">{output.runtimeMs}</span>
                            ...
                            {output.status === 'success' && (
                              <>
                                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden mb-2">
                                   <motion.div 
                                      initial={{ width: 0 }} 
                                      animate={{ width: `${output.runtimePercentile}%` }}
                                      className="h-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]" 
                                   />
                                </div>
                                <div className="text-xs text-slate-400">
                                   Beats <span className="text-blue-400 font-bold">{output.runtimePercentile}%</span> of users
                                </div>
                              </>
                            )}
                         </div>
                         ...
                         <div className="glass-panel border border-white/5 p-5 rounded-2xl relative overflow-hidden group">
                            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                               <Maximize2 className="w-12 h-12 text-purple-400" />
                            </div>
                            <div className="text-[10px] uppercase font-black text-slate-500 tracking-widest mb-1">Memory Usage</div>
                            <div className="flex items-baseline gap-2 mb-3">
                               <span className="text-2xl font-black text-white">{Math.round(output.memoryKb / 102.4) / 10}</span>
                            ...
                            {output.status === 'success' && (
                              <>
                                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden mb-2">
                                   <motion.div 
                                      initial={{ width: 0 }} 
                                      animate={{ width: `${output.memoryPercentile}%` }}
                                      className="h-full bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.5)]" 
                                   />
                                </div>
                                <div className="text-xs text-slate-400">
                                   Beats <span className="text-purple-400 font-bold">{output.memoryPercentile}%</span> of users
                                </div>
                              </>
                            )}
                         </div>
                      ...
                      <div className="bg-[#1e1e1e] rounded-2xl p-6 border border-white/5 space-y-4">
                         <div className="flex items-center justify-between border-b border-white/5 pb-4">
                            <div className="flex items-center gap-3">
                               ...
                               <div>
                                  <div className="text-[10px] uppercase font-black text-slate-500 tracking-widest">Test Summary</div>
                                  <div className="text-sm font-bold text-white">{output.passedCount} / {output.totalCount} Cases Passed</div>
                                </div>
                            </div>
                            <button 
                               onClick={() => setActiveTab('diagnostics')}
                            ...
                            >
                               View Detailed Results <ChevronRight className="w-3 h-3" />
                            </button>
                         </div>
                         
                         <div className="space-y-2">
                            <div className="text-[10px] uppercase font-black text-slate-500 tracking-widest">Standard Output</div>
                            <div className="whitespace-pre-wrap font-mono text-xs text-slate-400 bg-black/40 p-4 rounded-xl border border-white/5 max-h-[200px] overflow-y-auto custom-scrollbar leading-relaxed">
